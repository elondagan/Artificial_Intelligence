...
class _RelaxationHeuristic:
    ...
    def __init__(self, task):
        ...
        for op in task.A:
            ...
            ro = RelaxedOperator(op, task.A[op]['pre'], task.A[op]['add'])
            self.operators.append(ro)
            # Initialize precondition_of-list for each fact
            for var in task.A[op]['pre']:
    ...
    def get_cost(self, operator, pre):
        ...
        return None, cost + 1  # operator.cost
...
class hAddHeuristic(_RelaxationHeuristic):  # VERY un-admissible
...
class hSAHeuristic(_RelaxationHeuristic):
    ...
    def get_cost(self, operator, pre):
        ...
        # heuristic value.
        unioned_sets.add(operator)
        return (unioned_sets, cost + 1)
